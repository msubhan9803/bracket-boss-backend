...
export class SingleEliminationStrategy implements FormatStrategy {
  ...
  async handleEndRound(poolId: number) {
    ...
    const rounds = await this.roundService.findRoundsByPoolId(poolId, ['matches', 'matches.winnerTeam']);
    const currentRoundIndex = rounds.findIndex(round => round.status === RoundStatusTypesEnum.in_progress);
    ...
    const incompleteMatches = matchesOfCurrentRound.filter(match => match.status !== MatchStatusTypes.completed);
    if (incompleteMatches.length > 0) {
      throw new Error(`Cannot end round ${currentRound.name} in ${level.name} as ${incompleteMatches.length} match(es) still incomplete`);
    }
    ...
    const draftMatchList = this.singleEliminationScheduleBuilderService.generateSingleEliminationMatches(winningTeams);
    const timeSlotWithCourts = await this.courtManagementService.getAvailableCourts(tournament.start_date, tournament.end_date);
